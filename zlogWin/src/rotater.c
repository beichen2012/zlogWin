...
#ifdef _MSC_VER
		if (a_rotater->lock_fd != INVALID_HANDLE_VALUE)
		{
...
static int zlog_rotater_seq_files(zlog_rotater_t * a_rotater)
{
...
#ifdef _MSC_VER
	int fileNums = zc_arraylist_len(a_rotater->files);

	//先判断是否只有一个文件
	if (fileNums == 1)
	{
		a_file = zc_arraylist_get(a_rotater->files, 0);
		if (!a_file)
		{
		...
		nwrite = snprintf(new_path, sizeof(new_path), "%.*s%0*d%s",
			...
			a_rotater->num_width, a_rotater->max_count - 1,
	...
	//如果有多个文件
	//首先删除掉第 1 个文件，然后 把文件依次改名
	if (fileNums >= a_rotater->max_count)
	{
		a_file = zc_arraylist_get(a_rotater->files, 1);
		if (!a_file)
		{
			zc_error("zc_arraylist_get fail");
			return -1;
		}
		SetFileAttributesA(a_file->path, 0);
		if (remove(a_file->path))
		{
			zc_error("zc_arraylist_get fail");
		}
	}
	...
	if(fileNums >= 2)
	{
		for (i = 1; i < zc_arraylist_len(a_rotater->files); i++)
		{//只改  1 ~ (len-1)个文件，第0个文件改名成 a.maxcount-1
			...
			nwrite = snprintf(new_path, sizeof(new_path), "%.*s%0*d%s",
				(int)a_rotater->num_start_len, a_rotater->glob_path,
				a_rotater->num_width, a_file->index - 1,
				a_rotater->glob_path + a_rotater->num_end_len);
			if (nwrite < 0 || nwrite >= sizeof(new_path)) {
				zc_error("nwirte[%d], overflow or errno[%d]", nwrite, errno);
				return -1;
	...
	if (rename(a_rotater->base_path, new_path)) {
		zc_error("rename[%s]->[%s] fail, errno[%d]", a_rotater->base_path, new_path, GetLastError());
		return -1;
	}
#endif // _MSC_VER
