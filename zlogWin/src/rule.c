...
#else
#define winopen(f,m,p) open(f,m,p)
#define winwrite(f,s,l) write(f,s,l)
#define winclose(f) close(f)
#endif
...
static int zlog_rule_output_dynamic_file_single(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	fd = winopen(zlog_buf_str(a_thread->path_buf),
		a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
...
static int zlog_rule_output_dynamic_file_rotate(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	fd = winopen(path, a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
		zc_error("open file[%s] fail, errno[%d]", zlog_buf_str(a_thread->path_buf), errno);
		return -1;
	}
...
#ifdef _MSC_VER
HANDLE STDOUT_FILENO = GetStdHandle(STD_OUTPUT_HANDLE);
#endif

	if (winwrite(STDOUT_FILENO,
...
#ifdef _MSC_VER
HANDLE	STDERR_FILENO =	GetStdHandle(STD_ERROR_HANDLE);
#endif // _MSC_VER
	if (winwrite(STDERR_FILENO,
		zlog_buf_str(a_thread->msg_buf), zlog_buf_len(a_thread->msg_buf)) < 0) {
		zc_error("write fail, errno[%d]", errno);
		return -1;
	}
		...
		int * time_cache_count)
{
	...
	case '|':
		...
		a_rule->pipe_fd = _fileno(a_rule->pipe_fp);
		if (a_rule->pipe_fd < 0) {
